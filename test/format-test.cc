...
TEST(format_test, display_width) {
  EXPECT_EQ(fmt::display_width(""), 0);
  EXPECT_EQ(fmt::display_width("hello"), 5);
  EXPECT_EQ(fmt::display_width("a中b"), 4);       // 1 + 2 + 1
  EXPECT_EQ(fmt::display_width("中中"), 4);       // 2 + 2
  EXPECT_EQ(fmt::display_width("e\u0301"), 1);    // combining mark is zero-width
  EXPECT_EQ(fmt::display_width("a\u200bb"), 2);   // ZWSP is zero-width
  EXPECT_EQ(fmt::display_width("🐱"), 2);         // wide emoji
  // Public entry point matches the detail helper.
  EXPECT_EQ(fmt::display_width("a中b"), fmt::detail::display_columns("a中b"));
}
